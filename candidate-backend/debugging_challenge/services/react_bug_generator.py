...
{{
    ...
    def analyze_skills_for_challenge(self, user_skills: Dict) -> Dict:
        ...
        try:
            ...
            prompt = SKILL_BASED_CHALLENGE_PROMPT.format(
                user_skills=skills_text
            )
            
            # Call LLM to analyze skills
            response = self.llm.invoke(prompt)
            ...
            if json_match:
                ...
                # Map the AI's recommendations to our enums
                language = result.get("recommended_language", "python")
                difficulty = result.get("difficulty", "medium")
                bug_focus = result.get("bug_focus", ["logic_error", "syntax_error"])
                
                # Determine number of bugs based on difficulty
                num_bugs = 4 if difficulty == "hard" else 3 if difficulty == "medium" else 2
                
                # Convert bug types to our enum
                bug_types = []
                for bug_type_str in bug_focus:
                    try:
                        bug_types.append(BugType(bug_type_str))
                    except:
                        # Default to logic error if unknown type
                        bug_types.append(BugType.LOGIC_ERROR)
                ...
                return {
                    ...
                    "educational_goals": result.get("educational_goals", "")
                }
            else:
                raise ValueError("No valid JSON in skill analysis response")
            ...
            return {
                ...
                "difficulty": "medium",
                "num_bugs": 3,
                "bug_types": [BugType.LOGIC_ERROR, BugType.SYNTAX_ERROR],
                "reasoning": "Using defaults due to analysis error",
                "educational_goals": "General debugging practice"
            }
