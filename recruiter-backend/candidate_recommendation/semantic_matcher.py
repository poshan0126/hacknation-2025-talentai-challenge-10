"""
Semantic matching between job descriptions and candidate resumes.
"""
...
from models import SimpleJobDescription, SimpleCandidateMatch
...
def _skills_from_jd(job: SimpleJobDescription) -> List[str]:
...
class SemanticMatcher:
    ...
    def match_candidates(
        self,
        job: SimpleJobDescription,
        parsed_resumes_dir: str,
        top_n: int = 10,
    ) -> List[SimpleCandidateMatch]:
        """
        ...
        results: List[SimpleCandidateMatch] = []
        for idx in ordered_idx[:top_n]:
            meta = metas[idx]
            results.append(
                SimpleCandidateMatch(
                    name=meta.get("name", ""),
